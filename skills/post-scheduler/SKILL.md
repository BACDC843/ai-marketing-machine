...
## Finding the tools this skill needs

**Find a connector by capability, not by name.** Connector prefixes differ between installs — the same Blotato server can appear as `mcp__Blotato__…`, `mcp__claude_ai_Blotato__…`, or something else entirely depending on how it was added. Search for the *capability* (a Blotato account-listing tool, a Blotato post-creating tool) and use whatever identifier resolves. **Never match a literal `mcp__…` string**, and never conclude a connector is absent because one guessed name failed.

If nothing resolves, Blotato is genuinely not connected — take the Step 6 fallback and say so once.

---

### Step 3: Fetch connected accounts

Call the Blotato **account-listing** tool (resolved per the section above). It returns the connected accounts and their platforms.
...
Call the Blotato **post-creating** tool (resolved per the section above) with:
...
If no Blotato tool resolves at all, or the account-listing call errors out (not signed in, server not configured, or no connected accounts):
...
**Paste the post directly into your reply, formatted for copying.** A social caption belongs in the conversation — it is about to be pasted into a phone. If the user explicitly asks for a file, write it into `Business/[slug]/social/` and state that verified path; never name a path you did not confirm, and never report a save into a session working directory that will be discarded.

Format it in the reply like this:
...
This skill works without Blotato. If you're not ready to set it up, it'll hand you the finished post right in the conversation, ready to copy-paste into each platform. You'll lose the scheduling automation but you'll still get the writing + grading.
