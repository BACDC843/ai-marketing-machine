...
Read the brand context resolved above — the business profile's `brand-voice.md` first, `brand-brief.md` only as a fallback. You'll grade voice match against that defined voice. Skip the voice-match dimension, and say so in the output, only when neither source exists.
...
| **Voice match (0-10)** | Does it sound like the voice defined in the brand context resolved above? Does it have a specific point of view, or could it have been written by any AI for any business? Generic voice = generic content = no virality. Skip and note only if no brand context exists at all. |
