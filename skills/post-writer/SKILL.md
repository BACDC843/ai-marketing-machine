...
1. Read the brand context resolved above. **If neither a business profile nor a `brand-brief.md` exists, automatically invoke the `brand-brief` skill to capture one now.** Don't tell the user to "run brand-brief first" — just walk them through it, save it, then continue.
...
**The hook is 50% of the post-grader score. If the hook is weak, the post can't recover.** Pick a pattern that fits the topic AND the user's wedge from their brand context.
...
> Why it works: forces the reader to pick a side. Polarity drives comments. Pull the wedge from their brand context.
...
One CTA per post. Not three. Pull from the brand context (the "Primary CTA" field, or the profile's `products.md`), but adapt to **what the platform's algorithm rewards** (see Step 4.5 below).
